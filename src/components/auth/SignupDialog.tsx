...
const SignupDialog: React.FC<SignupDialogProps> = ({ isOpen, onClose }) => {
  ...
  const [step, setStep] = useState<'google' | 'generate' | 'download' | 'profile'>('google');
  ...
  const handleGoogleSignup = () => {
    toast({ title: 'Google ile kaydol!', description: 'Giriş sayfasına yönlendiriliyorsun.' });
    onClose();
  };
  ...
  const downloadKey = () => {
    try {
      ...
      const filename = `nostr-${location.hostname.replace(/\\./g, '-')}-${npub.slice(5, 9)}.nsec.txt`;
  ...
  const getTitle = () => {
    if (step === 'google') return 'Google ile Kaydol';
      ...
      <DialogContent className="max-w-[95vw] sm:max-w-sm max-h-[90dvh] p-0 gap-6 overflow-hidden rounded-2xl overflow-y-auto">
        ...
        {step === 'google' && (
          <div className="px-6 pb-6 space-y-4">
            <Button 
              className="w-full h-14 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-semibold shadow-lg"
            ...
            >
              <span className="flex items-center gap-2">
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  ...
                  <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.44 1 12.5s.43 3.95 1.18 5.43l3.66-2.84z"/>
                  <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
                Google ile Kaydol
              </span>
            </Button>
            <Button className="w-full" variant="outline" onClick={() => setStep('generate')}>
              Nostr ile Kaydol
            </Button>
          </div>
        )}
