        ...
        <div className='flex flex-col items-center space-y-4'>
          {connectError ? (
            ...
            <div className='flex items-center justify-center h-[100px]'>
              <Loader2 className='w-8 h-4 animate-spin text-muted-foreground' />
            </div>
          )}
        </div>
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[95vw] sm:max-w-sm max-h-[90dvh] p-0 gap-6 overflow-hidden rounded-2xl overflow-y-auto">
        ...
        <div className='px-6 pb-6 space-y-4 overflow-y-auto'>
          {/* Google Login - NO CUSTOM BUTTON, PURE GSI */}
          <div className="space-y-3">
            <div id="google-signin-button" className="w-full h-14 rounded-xl" />
            {!googleAuth.isSignedIn && (
              <p className="text-xs text-muted-foreground text-center">
                Drive erişimiyle verileriniz senkronize kalır
              </p>
            )}
          </div>

          {/* Nostr Options - Collapsible */}
